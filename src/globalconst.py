...
def reverse_dict(m):
    d = {}
    keys = [k for k, _ in m.iteritems()]
    values = [v for _, v in m.iteritems()]
...
def partial_similarity(player, patt_lst, patt_len, rem_lst, rem_len):
    # reachable squares are a list of valid squares that can be reached
    # from the current square
    reachable = BLACKMAP if player == BLACK else WHITEMAP
    # try all permutations of the pattern and see which gives the highest
    # similarity to the input set
    largest_sum = 0.0
    for p in itertools.permutations(patt_lst):
        working_set = set(rem_lst)
        num_reachable = rem_len
        dist_sum = 0.0
        for i, a in enumerate(p):
            dist = 0.0
            coord = None
            for b in working_set:
                # for a checker in the pattern to be considered similar to one
                # in the input set, it must be able to be moved to that square.
                if a in reachable[b]:
                    # translate checker index to grid X,Y coordinates in order
                    # to do a distance calculation
                    ga, gb = grid[a], grid[b]
                    # the inverse of squared distance means that less similar
                    # squares will return a smaller distance value.
                    v = 1.0 / square_distance(ga, gb)
                    if v > dist:
                        dist = v
                        coord = b
                else:
                    num_reachable -= 1
            # end search early if, for a given permutation, the number of squares in
            # the pattern that can be reached falls below the number remaining in the input set.
            if num_reachable < patt_len - i:
                break
            if coord:
                working_set.remove(coord)
                dist_sum += dist
        largest_sum = max(largest_sum, dist_sum)
    return largest_sum

def similarity(player, pattern, board):
    """
    Returns a similarity score that tells how well a given pattern of checkers matches the
    current board configuration for a particular player.

    player: either WHITE or BLACK.
    pattern: a dict of index, value pairs of checkerboard squares to match against
    board: a list of checkerboard squares
    """
    if len(input_set) < len(pattern):
        return 0.0   # if not enough items to match the pattern, then there's no similarity
    num_matches = sum(1 for idx, sq in pattern if board[idx] == sq)
    sq_matches = num_matches * num_matches
    # patt_lst is all remaining squares in the pattern after removing the
    # exact matches
    patt_lst = [i for i in pattern - exact_matches]
    patt_len = len(patt_lst)
    # rem_lst is all remaining squares in the input set after removing the
    # exact matches
    rem_lst = [i for i in input_set - exact_matches]
    rem_len = len(rem_lst)
    # get sum of all partial similarity scores
    sq_partial = partial_similarity(player, patt_lst, patt_len, rem_lst, rem_len)
    if sq_partial > 0.0:
        return sq_matches + sq_partial # total squared sum
    else:
